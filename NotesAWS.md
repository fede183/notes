...
m: instance class, in this case is general porpuse
...
if you see a timeout in EC2, it will be releated to the security group config
...
EC2 reserved convertable
	same but you can converte instance details
EC2 saving plans
	flexible size
...
can be attach with multiple instances witthin the same zone
...
highly availablñe, scalable, expensive
...
set throuhput
#### Elastic
automathicaly scales throughput based on workload
...
it ask the instance, normaly with health and if it doesn't get an OK(200) is unhealthy
...
millons req per second
static ip <-> AZ
works with EC2, ip private, application load balancers
health checks http, tcp

## Gateway Load Balancer: 3 layers - ip protocal
they can also be internal or external
firewalls, intruction detection
3er party
works with EC2, ip private
it redirects your traffic to a 3er party target group and theit responce to your application
...
application cookie: genetated by Load Balancer
...
## Connection draining
time to complete in-fligh request while instance is de-register or un healthy
1 to 3600 seconds
can be disable
...
re-create or termnates instances
...
you have to set maximun storage threshold

## Read replicas
must update connection string for laverage
network lost
...
serveless
auto scaling
higly available (multi-AZ)
no code change required
never publicaly accessed

## Elastic Cache
it manage regis or memcache
...
### memcache
multi-mode for partioning of data
multi-thredded architecture

### Lazy loading/cache aside
read, bit fubd in cache, write in cache
...
### MemoryDb for Redis
Redis-cinoatible, durable, in-memor database service
ultra-fast
...
assing a weight of 0 to a record to stop sending traffic, if all are 0, all will be return equally

## traffic(%) = weight of record/sum of all weight

## Routing Policies - Latency based
redired to the resource that has least latency
...
## Geoproximity
reoute traffic based on the geographic location of users and locations
more traffic on locations based on bias

## Routing policies - IP based routing
based on IP addresses
you provide a list of CIDRs for your clients and the correspoding endpoints locations
...
### Subnet
allows to partion your network inside your VPC
...
## VPC endpoint
connects AWS services using private network insted of public
site to site VPN
...
EC2 instances to learn about themselfs
	name 
	ip
	IAM role
...
## AWS CLI credentials privide chain
...
lifecycle rules
	transition actions:
		...
		move to Glacier for archiving after 6 mounths
	expiration actions:
...
needs IAM permitions to send the notifications or access policies for the receivers
...
Retrive less data using SQL by performing server
side filtering
...
use cached key, if not find is forwared
...
is basicaly an way to complete the request after take the cache

## Cache invalidation
you can invalidates the totality or partialy independent of TTL
Example you update the content

## Cache Behaviors
expecific of content type or nature of request
...
### Binpack
place based on the least avaliable amount of CPU or memory
...
example: instance ID, attribute: ecs.avaliability-zone
...
## EKS data volumes
needs storage class
laverages a container storage interface (CSI)
supports:
+ EBS
+ EFS
+ FSX for Lustre
...
|                 | Acale based on the number of SQS messages                   |
